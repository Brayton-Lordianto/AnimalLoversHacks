...
let PetMeArtifact = require('./PetMe.json')

const signerId = AccountId.fromString(process.env.NISCHAL_ID);
const signerKey = PrivateKey.fromString(process.env.NISCHAL_PVKEY); // TO WORK WITH HETHERS, IT MUST BE ECDSA KEY (FOR NOW)
...
async function main() {
    ...
    const bytecode = fs.readFileSync("./PetMe_sol_PetMe.bin").toString();
    const abi = [
    ...
    const factory = new hethers.ContractFactory(PetMeArtifact.abi, bytecode, wallet);
